...
class CautionDocxExportTest(
    ...
    @Test
    fun `prints the caution's signature block with upper-case titles and abbreviated civilities`() {
        val dcm = dcmMemberId()
        val client =
            clients.create(
                CreateClientCommand(
                    ...
                    raisonSociale = "GUINEENNE DES TRAVAUX ET FOURNITURES - SARLU",
                    createdBy = dcm,
                    // Entered in mixed case; the model prints the agency line upper-cased.
        ...
        val text = allText(export.export(created.id).content)

        // "Agence Principale" must not render as "AGENCE AGENCE PRINCIPALE".
        assertContains(text, "AFRILAND FIRST BANK ; AGENCE PRINCIPALE")
        // Body prose keeps the full civility, the signature block abbreviates it.
        assertContains(text, "Monsieur QUENTIN DETCHENOU, Directeur Crédit Marketing")
        assertContains(text, "Madame FANNY SOUMAH, Directrice Générale Adjointe")
        // The models print the titles upper-cased AND unaccented ("CREDIT", not "CRÉDIT").
        assertContains(text, "DIRECTEUR CREDIT MARKETING")
        assertContains(text, "DIRECTRICE GENERALE ADJOINTE")
        assertContains(text, "M. QUENTIN DETCHENOU")
        assertContains(text, "Mme. FANNY SOUMAH")
    }
